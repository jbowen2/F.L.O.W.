 ...
 */
module.exports = (message,redraw) =>{

    const guild = message.guild;
    const author = guild.members.resolve(message.author.id);
	
    con.query(`SELECT * FROM tracker ORDER BY Created_at DESC WHERE Guild_id`, async function (err, result, fields) {
        if (err) throw err;
        ...
        con.query(`INSERT INTO message (Message_id, Discord_id, Tracker_id) VALUES (${message.id},${author.id},${result[0].Message_id})`, function (err, result, fields) {
            if (err) throw err;
            ...
            con.query(`SELECT * FROM users u WHERE guild_id = ${guild.id} ORDER BY Discord_id DESC`, async function (err, result, fields) {
                if (err) throw err;
                for(const user of result){
                    ...
                    if(user.has_posted){
                        con.query(`SELECT m.Created_at
                            ...
                            ORDER BY m.Created_at DESC`, function (err, result, fields) {
                            if (err) throw err;
                ...
                setTimeout( function() { 
                    if(redraw){
                        ...
                        guild.channels.resolve(tracker).send(report); 
                    }
                }, 3000);
